...
function loadState(){
  ...
  // The authenticated cloud primary is the runtime authority. The rolling backup
  // intentionally contains the previous revision and is recovery-only; silently
  // promoting it can resurrect deleted or superseded documents after a refresh.
...
const PRIMARY_TABS = ['create','documents','manual','recovery'];
const TAB_ROUTES = { create:'#/create-new', documents:'#/all-documents', manual:'#/user-manual', recovery:'#/recovery' };
const ROUTE_TABS = Object.fromEntries(Object.entries(TAB_ROUTES).map(([tab,route])=>[route,tab]));
const LEGACY_TAB_REDIRECTS = { overview:'create', approvals:'documents', activity:'recovery', guide:'manual' };
const LEGACY_ROUTE_REDIRECTS = { '#/overview':'create', '#/create':'create', '#/documents':'documents', '#/approvals':'documents', '#/activity':'recovery', '#/guide':'manual' };
...
function setTab(tab){ const resolved=LEGACY_TAB_REDIRECTS[tab]||tab; ui.tab=PRIMARY_TABS.includes(resolved)?resolved:'create'; ui.modal=null; syncRoute(ui.tab); globalThis.WMModuleStore.setItem(UI_KEY,JSON.stringify({...ui,form:undefined,errors:{},modal:null,companyPanelOpen:false})); render(); window.scrollTo({top:0,behavior:'smooth'}); }
...
function render(){
  ...
  app.innerHTML=`<div class="app-shell">
    <header class="topbar"><a class="brand-lockup" href="#/create-new" data-tab="create" aria-label="TradeLink home"><span class="brand-mark" aria-hidden="true"><i></i></span><span class="brand-copy"><strong>TradeLink</strong><small>PORTAL</small></span></a>
    <nav class="nav-tabs" aria-label="Primary navigation">${navButton('create','＋  Create New')}${navButton('documents','☷  All Documents')}${navButton('manual','▤  User Manual')}${navButton('recovery','◈  Recovery')}</nav>
    <button class="vendor-pill" id="vendorTrigger" type="button" aria-haspopup="dialog" aria-expanded="${ui.companyPanelOpen}"><span class="vendor-pill-icon" aria-hidden="true">▣</span><span class="vendor-pill-name">${esc(vendorShortName(selectedVendor()))}</span><span class="vendor-pill-chevron" aria-hidden="true">⌄</span></button></header>
    <main id="mainView">${renderView()}</main>${ui.companyPanelOpen?renderCompanyPanel():''}<div id="modalHost"></div></div>`;
...
function navButton(id,label){ return `<button data-tab="${id}" class="${ui.tab===id?'active':''}">${label}</button>`; }
...
function hero(kicker,title,accent,lede,side=''){ return `<div class="intro-grid"><div><p class="eyebrow">${kicker}</p><h1>${title} <em>${accent}</em></h1><p class="lede">${lede}</p></div><div class="date-stack"><span>${fmtDate(new Date())}</span><strong>${side||selectedVendor().name.split(' ').slice(0,2).join(' ')}</strong></div></div>`; }
...
function renderCreateTypeTabs(active){ const order=['esi','packing','delivery','payment','quotation','po']; return `<div class="create-type-tabs" role="tablist" aria-label="Create document type">${order.map(type=>`<button type="button" role="tab" aria-selected="${active===type}" class="${active===type?'active':''}" data-create-type="${type}"><span>${esc(TYPES[type].short)}</span>${esc(createTypeLabel(type))}</button>`).join('')}</div>`; }
...
function renderRecovery(){
  ...
  return `<section class="view">${hero(pane==='activity'?'ACTIVITY':'DISASTER RECOVERY',pane==='activity'?'Every change,':'Cloud-backed,',pane==='activity'?'easy to trace.':'recoverable by design.',pane==='activity'?'Find document changes, workflow decisions, comments, backups, restores, and other recorded actions without leaving the recovery module.':'TradeLink keeps an explicit cloud recovery copy and supports named snapshots, integrity hashes, export/import, and controlled reset.',pane==='activity'?`${state.audit.length} recorded events`:`${state.snapshots.length} snapshots`)}<div class="recovery-tabs" role="tablist" aria-label="Recovery sections"><button role="tab" aria-selected="${pane==='tools'}" class="${pane==='tools'?'active':''}" data-recovery-pane="tools">Recovery Tools</button><button role="tab" aria-selected="${pane==='activity'}" class="${pane==='activity'?'active':''}" data-recovery-pane="activity">Activity</button></div>${content}</section>`;
}
...
function renderActivityInPlace(){
  ...
  workspace.outerHTML=renderActivityWorkspace(); bindActivityControls();
...
function renderModal(type,id){ ui.modal={type,id}; const host=document.querySelector('#modalHost'); if(!host)return; if(type==='preview'){
  ...
  document.querySelectorAll('[data-close-modal]').forEach(el=>el.addEventListener('click',closeModal)); requestAnimationFrame(()=>stableFocus(document.querySelector('.modal-close'))); document.querySelector('[data-print]')?.addEventListener('click',()=>generateDocumentPdf(id)); document.querySelector('#commentForm')?.addEventListener('submit',e=>{e.preventDefault();addComment(id,new FormData(e.currentTarget).get('comment')||'')});
  }
}
