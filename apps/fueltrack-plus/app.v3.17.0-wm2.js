(() => {
  ...
  function roleAssignmentAllowed(){return{ok:false,message:"Role assignments are managed centrally in Work Management Users."};}
  function setUserRole(){toast("Managed centrally","Open Work Management Users to change account roles.","info");}
  function addManagedUser(){toast("Managed centrally","Create and manage accounts from Work Management Users.","info");return false;}
  function deleteManagedUser(){toast("Managed centrally","Disable or change accounts from Work Management Users.","info");}
  ...
  function hydrateState(options = {}) {
    ...
    state.storageErrors = [requests.error, activity.error, prefs.error, inventory.error, userRoles.error].filter(Boolean);
  ...
  function renderActivity() {
    ...
    els.content.innerHTML = `
      ...
      ${filtered.length > visible.length ? `<div class="activity-load-more"><button class="button secondary" type="button" data-action="activity-load-more">Load ${Math.min(30,filtered.length-visible.length)} more</button></div>` : ""}
    `;
  ...
  function bindActivityControls() {
    ...
    els.content.querySelector('[data-action="activity-refresh"]')?.addEventListener("click",async()=>{const btn=els.content.querySelector('[data-action="activity-refresh"]');if(btn)btn.disabled=true;try{await globalThis.WMModuleActivity?.refresh?.();state.activity=[...(globalThis.WMModuleActivity?.items||[])];renderActivity();toast("Activity refreshed","The latest shared audit events were loaded from Supabase.","success");}catch(error){toast("Activity refresh failed",error?.message||"The activity stream could not be refreshed.","error");}finally{if(btn)btn.disabled=false;}});
    els.content.querySelector('[data-action="activity-export"]')?.addEventListener("click",()=>exportActivityPdf(filteredActivityItems()));
    els.content.querySelector('[data-action="activity-load-more"]')?.addEventListener("click",()=>{
      state.activityVisibleCount+=30; renderActivity();
    });
