(() => {
  ...
  function renderActivity() {
    ...
    els.content.innerHTML = `
      ...
      <div class="page-head activity-page-head">
        ...
        <div class="page-actions">
          <button class="button secondary" type="button" data-action="activity-refresh">↻ Refresh</button>
      ...
      <div class="activity-context-line">
      ...
      <section class="activity-stream" aria-label="Activity timeline">
        ${filtered.length
  ...
  function filteredActivityItems() {
    ...
    const rows=authorizedActivity().filter(a=>{
      const requestId=inferActivityRequestId(a);
      const hay=[a.title,a.message,a.actor,a.type,requestId].join(" ").toLowerCase();
    ...
    return `<article class="activity-event-card">
      ...
      <div class="activity-event-main">
        <div class="activity-event-topline">
          ...
          <time datetime="${escapeAttr(a.at||"")}">${escapeHtml(formatDateTime(new Date(a.at)))}</time>
        </div>
  ...
  function bindActivityControls() {
    const debounce=(fn,wait=180)=>{let timer;return(...args)=>{clearTimeout(timer);timer=setTimeout(()=>fn(...args),wait);};};
    const rerenderSearch=(value,caret)=>{state.activityFilters.search=value;state.activityVisibleCount=30;persistActivityFilters();renderActivity();requestAnimationFrame(()=>{const input=$("activitySearch");if(input){input.focus({preventScroll:true});const pos=Math.min(Number(caret)||value.length,input.value.length);try{input.setSelectionRange(pos,pos);}catch{}}});};
    $("activitySearch")?.addEventListener("input",debounce(e=>rerenderSearch(e.target.value,e.target.selectionStart)));
    $("activityType")?.addEventListener("change",e=>{state.activityFilters.type=e.target.value;state.activityVisibleCount=30;persistActivityFilters();renderActivity();});
    $("activityActor")?.addEventListener("change",e=>{state.activityFilters.actor=e.target.value;state.activityVisibleCount=30;persistActivityFilters();renderActivity();});
    $("activityPeriod")?.addEventListener("change",e=>{state.activityFilters.period=e.target.value;state.activityVisibleCount=30;persistActivityFilters();renderActivity();});
    $("activityLinkage")?.addEventListener("change",e=>{state.activityFilters.linkage=e.target.value;state.activityVisibleCount=30;persistActivityFilters();renderActivity();});
    $("activitySort")?.addEventListener("change",e=>{state.activityFilters.sort=e.target.value;state.activityVisibleCount=30;persistActivityFilters();renderActivity();});
    $("activitySearch")?.addEventListener("keydown",e=>{
      if(e.key==="Escape" && state.activityFilters.search) {
        e.preventDefault();
        state.activityFilters.search="";
        state.activityVisibleCount=30;persistActivityFilters();
        renderActivity();
        setTimeout(()=>$("activitySearch")?.focus(),0);
      }
    });
    els.content.querySelector('[data-action="activity-clear"]')?.addEventListener("click",()=>{
      ...
      state.activityVisibleCount=30; persistActivityFilters(); renderActivity();
    });
    const refreshActivityStream=async(action="activity-refresh")=>{const btn=els.content.querySelector(`[data-action="${action}"]`);if(btn)btn.disabled=true;try{await globalThis.WMModuleActivity?.refresh?.();state.activity=[...(globalThis.WMModuleActivity?.items||[])];state.activityError=null;state.loadedAt=new Date().toISOString();renderActivity();toast("Activity refreshed","The latest shared audit events were loaded from Supabase.","success");}catch(error){state.activityError=error?.message||"The activity stream could not be refreshed.";renderActivity();toast("Activity refresh failed",state.activityError,"error");}finally{if(btn?.isConnected)btn.disabled=false;}};
    els.content.querySelector('[data-action="activity-refresh"]')?.addEventListener("click",()=>refreshActivityStream("activity-refresh"));
    els.content.querySelector('[data-action="activity-retry"]')?.addEventListener("click",()=>refreshActivityStream("activity-retry"));
    ...
    els.content.querySelector('[data-action="activity-load-more"]')?.addEventListener("click",async()=>{
      ...
      if(localFiltered.length>state.activityVisibleCount){state.activityVisibleCount+=30;renderActivity();return;}
      if(!globalThis.WMModuleActivity?.hasMore)return;
      const btn=els.content.querySelector('[data-action="activity-load-more"]');if(btn)btn.disabled=true;
      try{await globalThis.WMModuleActivity.loadOlder();state.activity=[...(globalThis.WMModuleActivity.items||[])];state.activityError=null;state.activityVisibleCount+=30;renderActivity();}
      catch(error){state.activityError=error?.message||"Older Activity events could not be loaded.";renderActivity();toast("Older Activity unavailable",state.activityError,"error");}
  ...
  function showActivityDetail(id) {
    ...
    els.activityDetailBody.innerHTML=`
      <div class="activity-detail-grid">
        ...
        ${detail("Occurred",formatDateTime(new Date(a.at)))}
  ...
  async function copyActivityEvent(id) {
    ...
    const text=[
      ...
      `Time: ${formatDateTime(new Date(a.at))}`,
