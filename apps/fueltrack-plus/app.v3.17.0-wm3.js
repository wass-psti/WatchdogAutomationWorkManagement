(() => {
  ...
  window.addEventListener("wm:activity-change",(event)=>{
    ...
    if(state.route==="activity") renderActivity({preserveScroll:true});
    ...
    els.content.innerHTML = `
      ...
      <div class="page-head activity-page-head">
        ...
        <div class="page-actions">
          ...
          <button class="button secondary" type="button" data-action="activity-refresh" ${state.activitySyncStatus==="refreshing"?"disabled aria-busy=\"true\"":""}>↻ Refresh</button>
          ${hasPermission("activity.export")?`${hasPermission("activity.export")?`<button class="button secondary" type="button" data-action="activity-export" ${filtered.length?"":"disabled"}>⇩ Export PDF</button>`:""}`:""}
        </div>
      </div>
      ...
      <section class="panel activity-toolbar" aria-label="Activity controls">
        <label class="search-box activity-search">
          <span aria-hidden="true">⌕</span>
          <input id="activitySearch" type="search" value="${escapeAttr(state.activityFilters.search)}" placeholder="Search event, request ID, actor, or message…" />
        </label>
        <select id="activityType" class="select" aria-label="Filter activity type">
        ...
        <select id="activityActor" class="select" aria-label="Filter actor">
          <option value="" ${!state.activityFilters.actor?"selected":""}>All actors</option>
          ${actorOptions.map(actor=>`<option value="${escapeAttr(actor)}" ${state.activityFilters.actor===actor?"selected":""}>${escapeHtml(actor)}</option>`).join("")}
        </select>
        <select id="activityPeriod" class="select" aria-label="Filter time period">
        ...
        <select id="activityLinkage" class="select" aria-label="Filter request linkage">
        ...
        <select id="activitySort" class="select" aria-label="Sort activity">
        ...
        <button class="button ghost activity-clear" type="button" data-action="activity-clear" ${activeFilterCount?"":"disabled"}>Clear filters${activeFilterCount?` (${activeFilterCount})`:""}</button>
      </section>

      <div class="activity-context-line" role="status" aria-live="polite" aria-atomic="true">
      ...
      <section class="activity-stream" aria-label="Activity timeline" aria-busy="${state.activitySyncStatus==="refreshing"?"true":"false"}">
        ${filtered.length
          ? grouped.map(group=>activityDateGroup(group)).join("")
          : emptyState("No activity found","Adjust or clear the current filters. Activity events are preserved and are not deleted by filtering.")}
      </section>

      ${(filtered.length > visible.length || globalThis.WMModuleActivity?.hasMore) ? `<div class="activity-load-more"><button class="button secondary" type="button" data-action="activity-load-more">${filtered.length > visible.length ? `Load ${Math.min(30,filtered.length-visible.length)} more` : "Load older events"}</button></div>` : ""}
    `;
  ...
  function filteredActivityItems() {
    ...
    const rows=authorizedActivity().filter(a=>{
      ...
      return (!q||hay.includes(q)) &&
        ...
        (!state.activityFilters.actor||String(a.actor||"")===state.activityFilters.actor) &&
        periodOk && linkageOk;
    });
  ...
  function activityActorValues() {
    return [...new Set(authorizedActivity().map(a=>String(a.actor||"").trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  }
  ...
  function bindActivityControls() {
    const debounce=(fn,wait=180)=>{let timer;return(...args)=>{clearTimeout(timer);timer=setTimeout(()=>fn(...args),wait);};};
    const rerenderSearch=(value,caret)=>{state.activityFilters.search=value;state.activityVisibleCount=30;queueActivityFilterPersistence();renderActivity({preserveScroll:true});requestAnimationFrame(()=>{const input=$("activitySearch");if(input){input.focus({preventScroll:true});const pos=Math.min(Number(caret)||value.length,input.value.length);try{input.setSelectionRange(pos,pos);}catch{}}});};
    $("activitySearch")?.addEventListener("input",debounce(e=>rerenderSearch(e.target.value,e.target.selectionStart)));
    $("activityType")?.addEventListener("change",e=>{state.activityFilters.type=e.target.value;state.activityVisibleCount=30;queueActivityFilterPersistence();renderActivity({preserveScroll:true});});
    $("activityActor")?.addEventListener("change",e=>{state.activityFilters.actor=e.target.value;state.activityVisibleCount=30;queueActivityFilterPersistence();renderActivity({preserveScroll:true});});
    $("activityPeriod")?.addEventListener("change",e=>{state.activityFilters.period=e.target.value;state.activityVisibleCount=30;queueActivityFilterPersistence();renderActivity({preserveScroll:true});});
    $("activityLinkage")?.addEventListener("change",e=>{state.activityFilters.linkage=e.target.value;state.activityVisibleCount=30;queueActivityFilterPersistence();renderActivity({preserveScroll:true});});
    $("activitySort")?.addEventListener("change",e=>{state.activityFilters.sort=e.target.value;state.activityVisibleCount=30;queueActivityFilterPersistence();renderActivity({preserveScroll:true});});
    $("activitySearch")?.addEventListener("keydown",e=>{
      if(e.key==="Escape" && state.activityFilters.search) {
        e.preventDefault(); state.activityFilters.search=""; state.activityVisibleCount=30;queueActivityFilterPersistence();
        renderActivity({preserveScroll:true}); setTimeout(()=>$("activitySearch")?.focus(),0);
      }
    });
    els.content.querySelector('[data-action="activity-clear"]')?.addEventListener("click",()=>{
      state.activityFilters={search:"",type:"all",actor:"",period:"all",linkage:"all",sort:"newest"};
      state.activityVisibleCount=30; queueActivityFilterPersistence(); renderActivity({preserveScroll:true});
      requestAnimationFrame(()=>$("activitySearch")?.focus({preventScroll:true}));
    });
    els.content.querySelector('[data-action="activity-refresh"]')?.addEventListener("click",()=>refreshActivityStream({announce:true}));
    els.content.querySelector('[data-action="activity-retry"]')?.addEventListener("click",()=>refreshActivityStream({announce:true}));
    els.content.querySelector('[data-action="activity-export"]')?.addEventListener("click",()=>exportActivityPdf(filteredActivityItems()));
    els.content.querySelector('[data-action="activity-load-more"]')?.addEventListener("click",async()=>{
      const localFiltered=filteredActivityItems();
      if(localFiltered.length>state.activityVisibleCount){state.activityVisibleCount+=30;renderActivity({preserveScroll:true});return;}
      if(!globalThis.WMModuleActivity?.hasMore)return;
      const btn=els.content.querySelector('[data-action="activity-load-more"]');if(btn){btn.disabled=true;btn.setAttribute("aria-busy","true");}
      try{await globalThis.WMModuleActivity.loadOlder();state.activity=[...(globalThis.WMModuleActivity.items||[])];state.activityError=null;state.activityRefreshError=null;state.activityVisibleCount+=30;state.activityLastRefreshAt=new Date().toISOString();renderActivity({preserveScroll:true});}
      catch(error){state.activityError=error?.message||"Older Activity events could not be loaded.";state.activityRefreshError=state.activityError;state.activitySyncStatus="error";renderActivity({preserveScroll:true});toast("Older Activity unavailable",state.activityError,"error");}
      finally{if(btn?.isConnected){btn.disabled=false;btn.removeAttribute("aria-busy");}}
    });
    els.content.querySelectorAll("[data-activity-request]").forEach(btn=>btn.addEventListener("click",()=>showRequestDetail(btn.dataset.activityRequest)));
    els.content.querySelectorAll("[data-activity-detail]").forEach(btn=>btn.addEventListener("click",()=>showActivityDetail(btn.dataset.activityDetail)));
    els.content.querySelectorAll("[data-copy-activity]").forEach(btn=>btn.addEventListener("click",()=>copyActivityEvent(btn.dataset.copyActivity)));
  }
  ...
  async function refreshActivityStream({announce=false,silent=false}={}) {
    ...
    if(!silent) renderActivity({preserveScroll:true});
  ...
  function configureActivityAutoRefresh(){
    ...
    activityRefreshTimer=setInterval(()=>{if(!document.hidden&&!document.querySelector("dialog[open]")&&!activityRefreshPromise) void refreshActivityStream({silent:true});},ACTIVITY_REFRESH_INTERVAL_MS);
  }
