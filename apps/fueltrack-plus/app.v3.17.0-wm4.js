(() => {
  ...
  function updateActivityActorOptions() {
    ...
    const markup = ['<option value="">All actors</option>', ...activityActorValues().map(actor=>`<option value="${escapeAttr(actor)}">${escapeHtml(actor)}</option>`)].join("");
  ...
  function updateActivityResults({ refreshActors = false } = {}) {
    ...
    bindActivityResultActions();
  }

  function bindActivityResultActions() {
    els.content.querySelector('[data-action="activity-load-more"]')?.addEventListener("click",async()=>{
      if (filteredActivityItems().length > state.activityVisibleCount) {
        state.activityVisibleCount+=30;
      } else if (globalThis.WMModuleActivity?.hasMore) {
        try { await globalThis.WMModuleActivity.loadOlder?.(); state.activity=[...(globalThis.WMModuleActivity?.items||[])]; }
        catch(error){ return toast("Older activity unavailable",error?.message||"Older audit events could not be loaded.","error"); }
      }
      updateActivityResults({refreshActors:true});
    }, { once:true });
    els.content.querySelectorAll("[data-activity-request]").forEach(btn=>btn.addEventListener("click",()=>showRequestDetail(btn.dataset.activityRequest), { once:true }));
    els.content.querySelectorAll("[data-activity-detail]").forEach(btn=>btn.addEventListener("click",()=>showActivityDetail(btn.dataset.activityDetail), { once:true }));
    els.content.querySelectorAll("[data-copy-activity]").forEach(btn=>btn.addEventListener("click",()=>copyActivityEvent(btn.dataset.copyActivity), { once:true }));
  }
  ...
  function bindActivityControls() {
    ...
    $("activityRefresh")?.addEventListener("click",async()=>{
      const btn=$("activityRefresh");
      if(btn?.getAttribute("aria-busy")==="true") return;
      if(btn){btn.disabled=true;btn.setAttribute("aria-busy","true");btn.textContent="Refreshing…";}
      try{
        await globalThis.WMModuleActivity?.refresh?.();
        state.activity=[...(globalThis.WMModuleActivity?.items||[])];
        updateActivityResults({refreshActors:true});
        toast("Activity refreshed","The latest shared audit events were loaded from Supabase.","success");
      }catch(error){
        toast("Activity refresh failed",error?.message||"The activity stream could not be refreshed.","error");
      }finally{
        if(btn){btn.disabled=false;btn.removeAttribute("aria-busy");btn.textContent="↻ Refresh";}
      }
    });
